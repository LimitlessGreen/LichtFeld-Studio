...
TEST_F(MinimalSortDebugTest, Step1_BasicSort) {
    ...
    auto data = gs::Tensor::from_vector({3.0f, 1.0f, 4.0f}, {3}, gs::Device::CUDA);
    ...
    try {
        ...
        std::cout << "  Values dtype: " << gs::dtype_name(vals.dtype()) << std::endl;
        ...
        std::cout << "  Indices dtype: " << gs::dtype_name(idx.dtype()) << std::endl;
...
TEST_F(MinimalSortDebugTest, Step3_CheckIndicesDtype) {
    ...
    if (idx.dtype() == gs::DataType::Int64) {
        std::cout << "✓ Indices are Int64" << std::endl;
    } else if (idx.dtype() == gs::DataType::Int32) {
    ...
    std::cout << "Device: " << gs::device_name(idx.device()) << std::endl;
...
TEST_F(MinimalSortDebugTest, Step4_MoveIndicesToCPU) {
    ...
    std::cout << "Before cpu(): dtype=" << gs::dtype_name(idx.dtype()) 
              << " device=" << gs::device_name(idx.device()) << std::endl;
    ...
    try {
        ...
        std::cout << "After cpu(): dtype=" << gs::dtype_name(idx_cpu.dtype()) 
                  << " device=" << gs::device_name(idx_cpu.device()) << std::endl;
        
        EXPECT_EQ(idx_cpu.device(), gs::Device::CPU);
...
TEST_F(MinimalSortDebugTest, Step6_DirectPointerAccess) {
    ...
    std::cout << "Dtype: " << gs::dtype_name(idx_cpu.dtype()) << std::endl;
    
    if (idx_cpu.dtype() == gs::DataType::Int64) {
    ...
    } else if (idx_cpu.dtype() == gs::DataType::Int32) {
...
TEST_F(MinimalSortDebugTest, Step7_CheckIndicesValidity) {
    ...
    auto data = gs::Tensor::from_vector({5.0f, 2.0f, 8.0f, 1.0f}, {4}, gs::Device::CUDA);
    ...
    std::cout << "Dtype: " << gs::dtype_name(idx_cpu.dtype()) << std::endl;
    std::cout << "Device: " << gs::device_name(idx_cpu.device()) << std::endl;
