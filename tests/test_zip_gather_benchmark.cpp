...
protected:
    ...
    BenchmarkResult benchmark_zip_gather_2(size_t data_size, size_t gather_size, int num_iterations = 100) {
        ...
        auto data1 = Tensor::rand({static_cast<int>(data_size)}, Device::CUDA);
        auto data2 = Tensor::rand({static_cast<int>(data_size)}, Device::CUDA);

        // Create random indices
        auto indices = (Tensor::rand({static_cast<int>(gather_size)}, Device::CUDA) * (data_size - 1)).to(DataType::Int32);
        ...
        for (int i = 0; i < num_iterations; ++i) {
            // Allocate outputs
            result1_zip = Tensor::empty({static_cast<int>(gather_size)}, Device::CUDA, DataType::Float32);
            result2_zip = Tensor::empty({static_cast<int>(gather_size)}, Device::CUDA, DataType::Float32);
    ...
    BenchmarkResult benchmark_zip_gather_3(size_t data_size, size_t gather_size, int num_iterations = 100) {
        ...
        auto data1 = Tensor::rand({static_cast<int>(data_size)}, Device::CUDA);
        auto data2 = Tensor::rand({static_cast<int>(data_size)}, Device::CUDA);
        auto data3 = Tensor::rand({static_cast<int>(data_size)}, Device::CUDA);
        ...
        for (int i = 0; i < num_iterations; ++i) {
            ...
            result1_zip = Tensor::empty({static_cast<int>(gather_size)}, Device::CUDA, DataType::Float32);
            result2_zip = Tensor::empty({static_cast<int>(gather_size)}, Device::CUDA, DataType::Float32);
            result3_zip = Tensor::empty({static_cast<int>(gather_size)}, Device::CUDA, DataType::Float32);
