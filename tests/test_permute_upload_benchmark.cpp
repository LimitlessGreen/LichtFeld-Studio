...
TEST_F(PermuteUploadBenchmarkTest, TransferBandwidthSummary) {
    print_separator("📊 TRANSFER BANDWIDTH SUMMARY");

    std::cout << "\n🎯 THEORETICAL LIMITS:" << std::endl;
    std::cout << "  PCIe 3.0 x16: 12 GB/s (unidirectional)" << std::endl;
    std::cout << "  PCIe 4.0 x16: 24 GB/s (unidirectional)" << std::endl;
    std::cout << "  PCIe 5.0 x16: 48 GB/s (unidirectional)" << std::endl;

    std::cout << "\n📊 TYPICAL OBSERVED:" << std::endl;
    std::cout << "  CPU → GPU: 8-10 GB/s (66-83% PCIe 3.0 efficiency)" << std::endl;
    std::cout << "  GPU → CPU: 4-8 GB/s (33-66% PCIe 3.0 efficiency)" << std::endl;
    std::cout << "  Asymmetry due to PCIe protocol overhead and system architecture" << std::endl;

    std::cout << "\n🔴 CURRENT BOTTLENECK (from logs):" << std::endl;
    std::cout << "  point_cloud_renderer.cpp:169: 5.48ms for 24MB = 4.4 GB/s ✓ reasonable" << std::endl;
    std::cout << "  rendering_pipeline.cpp:269:   72ms for 20MB = 0.28 GB/s 🔴 TERRIBLE!" << std::endl;
    std::cout << "  → The 72ms is NOT from transfer, it's from CPU permute!" << std::endl;

    std::cout << "\n💡 KEY INSIGHT:" << std::endl;
    std::cout << "  If transfer benchmarks show good bandwidth (>5 GB/s)," << std::endl;
    std::cout << "  but production logs show 72ms, the bottleneck is NOT the transfer!" << std::endl;
    std::cout << "  It's the CPU permute operation before the transfer." << std::endl;
    std::cout << "  Solution: Fused kernel that permutes DURING transfer." << std::endl;
...
TEST_F(PermuteUploadBenchmarkTest, SummaryReport) {
    print_separator("📊 PERMUTE + UPLOAD BOTTLENECK SUMMARY");

    std::cout << "\n🔴 CRITICAL BOTTLENECK IDENTIFIED:" << std::endl;
    std::cout << "  Location: rendering_pipeline.cpp:269" << std::endl;
    std::cout << "  Code:     result.image = image_cpu.permute({2, 0, 1}).cuda();" << std::endl;
    std::cout << "  Current:  ~72ms in production logs!" << std::endl;
    std::cout << "  Target:   < 5ms (15x speedup needed)" << std::endl;

    std::cout << "\n📊 ROOT CAUSE ANALYSIS:" << std::endl;
    std::cout << "  1. CPU Permute is SLOW:" << std::endl;
    std::cout << "     - Element-by-element copy with index calculations" << std::endl;
    std::cout << "     - See tensor_shape_ops.cpp:86-106" << std::endl;
    std::cout << "     - Uses nested loops: for each element, calculate coords, copy" << std::endl;
    std::cout << "     - ~20M elements * (coord calc + index math) = SLOW" << std::endl;

    std::cout << "\n  2. Sequential Upload is INEFFICIENT:" << std::endl;
    std::cout << "     - Creates intermediate permuted buffer on CPU" << std::endl;
    std::cout << "     - Then uploads entire buffer sequentially" << std::endl;
    std::cout << "     - PCIe efficiency: ~2-5% (only 280 MB/s vs 12 GB/s capable)" << std::endl;

    std::cout << "\n  3. GPU Permute ALSO GOES TO CPU:" << std::endl;
    std::cout << "     - See tensor_shape_ops.cpp:81-84" << std::endl;
    std::cout << "     - if (device_ == Device::CUDA) { download → permute → upload }" << std::endl;
    std::cout << "     - Triple penalty: download + CPU permute + upload" << std::endl;

    std::cout << "\n💡 SOLUTION: Fused CUDA Kernel" << std::endl;
    std::cout << "  Write a custom kernel that:" << std::endl;
    std::cout << "  1. Reads from CPU pinned memory (fast DMA)" << std::endl;
    std::cout << "  2. Flips vertically (OpenGL origin)" << std::endl;
    std::cout << "  3. Transposes HWC → CHW" << std::endl;
    std::cout << "  4. Writes to GPU with coalesced access" << std::endl;
    std::cout << "  5. Uses warp-level primitives for efficiency" << std::endl;
    std::cout << "  All in ONE kernel launch!" << std::endl;

    std::cout << "\n📈 EXPECTED IMPROVEMENT:" << std::endl;
    std::cout << "  Current:  72ms (280 MB/s, 2% PCIe efficiency)" << std::endl;
    std::cout << "  Target:   < 5ms (4 GB/s, 33% PCIe efficiency)" << std::endl;
    std::cout << "  Speedup:  15x faster!" << std::endl;

    std::cout << "\n✅ RUN BENCHMARKS ABOVE TO VALIDATE CURRENT PERFORMANCE" << std::endl;
    std::cout << std::string(120, '=') << std::endl;
}
