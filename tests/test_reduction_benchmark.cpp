...
TEST_F(ReductionBenchmarkTest, SumReductionSingleDim) {
    ...
    for (const auto& [name, shape, dim] : test_cases) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            {
                Timer timer;
                auto result = tensor_custom.sum({static_cast<size_t>(dim)}, false);
                cudaDeviceSynchronize();
...
TEST_F(ReductionBenchmarkTest, MeanReduction) {
    ...
    for (const auto& [name, shape, dim] : test_cases) {
        ...
        for (int i = 0; i < iterations; ++i) {
            {
                Timer timer;
                auto result = tensor_custom.mean({static_cast<size_t>(dim)}, false);
                cudaDeviceSynchronize();
                total_custom += timer.elapsed_ms();
            }
    ...
    for (const auto& [name, shape, dim] : test_cases) {
        ...
        for (int i = 0; i < iterations; ++i) {
            if (is_min) {
                {
                    Timer timer;
                    auto result = tensor_custom.min({static_cast<size_t>(dim)}, false);
                    cudaDeviceSynchronize();
                    total_custom += timer.elapsed_ms();
                }
                ...
                {
                    Timer timer;
                    auto result = tensor_custom.max({static_cast<size_t>(dim)}, false);
                    cudaDeviceSynchronize();
                    total_custom += timer.elapsed_ms();
                }
