...
namespace gs::rendering {
    ...
    inline std::function<char*(size_t)> resize_function_wrapper_tensor(Tensor& t) {
        return [&t](size_t N) -> char* {
            if (N == 0) {
                t = Tensor::empty({0}, Device::CUDA, DataType::UInt8);
                return nullptr;
            }
            t = Tensor::empty({N}, Device::CUDA, DataType::UInt8);
    ...
    forward_wrapper_tensor(
        ...
        Tensor per_primitive_buffers = Tensor::empty({0}, Device::CUDA, DataType::UInt8);
        Tensor per_tile_buffers = Tensor::empty({0}, Device::CUDA, DataType::UInt8);
        Tensor per_instance_buffers = Tensor::empty({0}, Device::CUDA, DataType::UInt8);
