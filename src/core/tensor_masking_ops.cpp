...
namespace gs {
    ...
    Tensor Tensor::index_select(int dim, const Tensor& indices, BoundaryMode mode) const {
        if (!is_valid() || !indices.is_valid() || indices.ndim() != 1)
            return {};
    ...
    static Tensor& scatter_impl(Tensor& t, int dim, const Tensor& idx,
        ...
        if (!t.is_valid() || !idx.is_valid() || !src.is_valid())
            return t;

        dim = (dim < 0) ? t.shape().rank() + dim : dim;
        if (dim < 0 || dim >= static_cast<int>(t.shape().rank()))
            return t;
        ...
        if (t.device() == Device::CUDA) {
            ...
            for (int i = 0; i < dim; ++i)
                outer *= t.shape()[i];
            for (size_t i = dim + 1; i < t.shape().rank(); ++i)
                inner *= t.shape()[i];
            ...
            for (size_t o = 0; o < outer; ++o) {
                for (size_t i = 0; i < idx.numel(); ++i) {
                    int pos = indices[i];
                    if (pos < 0 || pos >= static_cast<int>(t.shape()[dim]))
                        continue;
    ...
    Tensor& Tensor::index_put_(const std::vector<Tensor>& indices, const Tensor& vals) {
        if (!is_valid() || indices.empty() || !vals.is_valid())
            return *this;
