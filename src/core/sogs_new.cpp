    ...
    std::expected<void, std::string> write_sog_new(
        ...
        try {
            ...
            for (int64_t i = 0; i < num_splats; ++i) {
                ...
                scales_data[ti * 4 + 0] = static_cast<uint8_t>(scales_labels_cpu[idx].item_int());
                scales_data[ti * 4 + 1] = static_cast<uint8_t>(scales_labels_cpu[num_splats + idx].item_int());
                scales_data[ti * 4 + 2] = static_cast<uint8_t>(scales_labels_cpu[2 * num_splats + idx].item_int());
            }
            ...
            for (int64_t i = 0; i < num_splats; ++i) {
                ...
                sh0_data[ti * 4 + 0] = static_cast<uint8_t>(colors_labels_cpu[idx].item_int());
                sh0_data[ti * 4 + 1] = static_cast<uint8_t>(colors_labels_cpu[num_splats + idx].item_int());
                sh0_data[ti * 4 + 2] = static_cast<uint8_t>(colors_labels_cpu[2 * num_splats + idx].item_int());
            ...
            if (sh_degree > 0 && shN.is_valid() && shN.numel() > 0) {
                ...
                if (sh_centroids.size(0) == 0) {
                                ...
                                for (int c = 0; c < 3; ++c) {
                                    ...
                                    if (centroid_idx < codebook_labels.size(0)) {
                                        centroids_buf[pixel_idx * 4 + c] =
                                            static_cast<uint8_t>(codebook_labels_cpu[centroid_idx].item_int());
                                    }
                                }
                            }
                        }
                    ...
                    for (int64_t i = 0; i < num_splats; ++i) {
                        int64_t idx = indices[i].item_as<int64_t>();
                        int32_t label = sh_labels_cpu[idx].item_int();
                        int ti = identity_layout(i, width);
            ...
            LOG_ERROR("Exception in write_sog_new: {}", e.what());
