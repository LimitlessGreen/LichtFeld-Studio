...
Tensor::Tensor(void* data, TensorShape shape, Device device, DataType dtype)
    : data_(data), shape_(shape), device_(device), dtype_(dtype), 
      owns_memory_(false), id_(next_id_++) {
...
Tensor Tensor::unsqueeze(int dim) const {
    if (dim < 0) dim = shape_.rank() + dim + 1;
...
Tensor Tensor::flatten(int start_dim, int end_dim) const {
    if (start_dim < 0) start_dim = shape_.rank() + start_dim;
    if (end_dim < 0) end_dim = shape_.rank() + end_dim;
...
TensorError::TensorError(const std::string& msg, const Tensor* t) 
    : std::runtime_error(msg), tensor_info_(t ? t->str() : "N/A") {}
