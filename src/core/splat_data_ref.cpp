...
#pragma omp parallel for if (num_points > 1000)
    ...
    void write_ply_impl(const gs::PointCloudRef& pc,
    ...
    std::filesystem::path write_sog_impl(const gs::SplatDataRef& splat_data,
                                         const std::filesystem::path& root,
                                         int iteration,
...
namespace gs {
    ...
    SplatDataRef::SplatDataRef(int sh_degree,
                         torch::Tensor means,
                         torch::Tensor sh0,
                         torch::Tensor shN,
    ...
    // Move constructor
    SplatDataRef::SplatDataRef(SplatDataRef&& other) noexcept
    ...
    SplatDataRef& SplatDataRef::operator=(SplatDataRef&& other) noexcept {
        if (this != &other) {
    ...
    SplatDataRef::~SplatDataRef() {
        wait_for_saves();
    }

    // Computed getters
    torch::Tensor SplatDataRef::get_means() const {
        return _means;
    }

    torch::Tensor SplatDataRef::get_opacity() const {
        return torch::sigmoid(_opacity).squeeze(-1);
    }

    torch::Tensor SplatDataRef::get_rotation() const {
        return torch::nn::functional::normalize(_rotation,
                                                torch::nn::functional::NormalizeFuncOptions().dim(-1));
    }

    torch::Tensor SplatDataRef::get_scaling() const {
        return torch::exp(_scaling);
    }

    torch::Tensor SplatDataRef::get_shs() const {
        return torch::cat({_sh0, _shN}, 1);
    }

    SplatDataRef& SplatDataRef::transform(const glm::mat4& transform_matrix) {
    ...
    if (std::abs(rotation.w - 1.0f) > 1e-6f) {
        ...
        auto w_result = w1 * w2 - x1 * x2 - y1 * y2 - z1 * z2;
        auto x_result = w1 * x2 + x1 * w2 + y1 * z2 - z1 * y2;
        auto y_result = w1 * y2 - x1 * z2 + y1 * w2 + z1 * x2;
        auto z_result = w1 * z2 + x1 * y2 - y1 * x2 + z1 * w2;

        _rotation.index_put_({torch::indexing::Slice(), 0}, w_result);
        _rotation.index_put_({torch::indexing::Slice(), 1}, x_result);
        _rotation.index_put_({torch::indexing::Slice(), 2}, y_result);
        _rotation.index_put_({torch::indexing::Slice(), 3}, z_result);
    }
    ...
    // Utility method
    void SplatDataRef::increment_sh_degree() {
        if (_active_sh_degree < _max_sh_degree) {
            _active_sh_degree++;
        }
    }

    void SplatDataRef::set_active_sh_degree(int sh_degree = 0) {
    ...
    std::vector<std::string> SplatDataRef::get_attribute_names() const {
    ...
    void SplatDataRef::wait_for_saves() const {
    ...
    void SplatDataRef::cleanup_finished_saves() const {
        std::lock_guard<std::mutex> lock(_save_mutex);
    ...
    // Export to PLY
    void SplatDataRef::save_ply(const std::filesystem::path& root, int iteration, bool join_threads, std::string stem) const {
    ...
    // Export to SOG
    std::filesystem::path SplatDataRef::save_sog(const std::filesystem::path& root, int iteration, int kmeans_iterations, bool join_threads) const {
    ...
    PointCloudRef SplatDataRef::to_point_cloud() const {
        PointCloudRef pc;
    ...
    std::expected<SplatDataRef, std::string> SplatDataRef::init_model_from_pointcloud(
        const param::TrainingParameters& params,
        torch::Tensor scene_center,
        const PointCloudRef& pcd) {
        ...
        try {
            ...
            std::println("Initialized SplatDataRef with:");
            ...
            return std::unexpected(std::format("Failed to initialize SplatDataRef: {}", e.what()));
        }
    }
    SplatDataRef SplatDataRef::crop_by_cropbox(const gs::geometry::BoundingBox& bounding_box) const {
        LOG_TIMER("SplatDataRef::crop_by_cropbox");

        if (_means.size(0) == 0) {
            LOG_WARN("Cannot crop empty SplatDataRef");
            return SplatDataRef(); // Return empty SplatData
        }
        ...
        if (points_inside == 0) {
            LOG_WARN("No points found inside bounding box, returning empty SplatDataRef");
            return SplatDataRef();
        }
        ...
        // Create new SplatDataRef with cropped tensors
        SplatDataRef cropped_splat(
            _max_sh_degree,
        ...
        LOG_DEBUG("Successfully cropped SplatDataRef: {} -> {} points (scale: {:.4f} -> {:.4f})",
