...
namespace lfs::training {
    ...
    void DefaultStrategy::remove_gaussians(const lfs::core::Tensor& mask) {
        if (mask.sum_scalar() == 0) {
            LOG_DEBUG("No Gaussians to remove");
            return;
        }

        LOG_DEBUG("Removing {} Gaussians", static_cast<int>(mask.sum_scalar()));
        remove(mask);
    }
    ...
    void DefaultStrategy::split(const lfs::core::Tensor& is_split) {
        // TODO: This function requires einsum and repeat operations
        // For now, log an error - we'll implement this after adding missing ops
        LOG_ERROR("DefaultStrategy::split() not yet implemented - requires einsum and repeat tensor ops");

        // Original logic requires:
        // 1. gsplat::quats_to_rotmats() - available from gsplat
        // 2. torch::einsum() - MISSING, need to implement or work around
        // 3. tensor.repeat() - MISSING, need to implement

        // We'll need to either:
        // a) Implement einsum for this specific case: "nij,nj,bnj->bni"
        // b) Manually implement the matrix multiplication this represents
        // c) Create a custom CUDA kernel for this operation
    }
