...
namespace ops {
    ...
    struct scalar_right_op {
        ...
        HOST_DEVICE constexpr scalar_right_op(T s) : scalar(s), op() {}
    ...
    struct scalar_left_op {
        ...
        HOST_DEVICE constexpr scalar_left_op(T s) : scalar(s), op() {}
    ...
    struct clamp_range_op {
        T min_val;
        T max_val;
        HOST_DEVICE constexpr clamp_range_op(T min_v, T max_v) : min_val(min_v), max_val(max_v) {}
        HOST_DEVICE constexpr T operator()(const T& x) const {
            #ifdef __CUDA_ARCH__
    ...
    struct index_clamp_op {
        ...
        HOST_DEVICE constexpr size_t operator()(int idx) const {
            if (idx < 0) idx += size;
            if (idx < 0) return 0;
            if (idx >= static_cast<int>(size)) return size - 1;
    ...
    struct iota_op {
        T start;
        T step;
        HOST_DEVICE constexpr iota_op(T s = T(0), T st = T(1)) : start(s), step(st) {}
    ...
    struct linspace_op {
        ...
        T end;
        size_t steps;
        HOST_DEVICE constexpr linspace_op(T s, T e, size_t n) : start(s), end(e), steps(n) {}
        HOST_DEVICE constexpr T operator()(size_t idx) const {
