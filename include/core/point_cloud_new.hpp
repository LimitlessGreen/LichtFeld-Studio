...
public:
    ...
    Tensor sh0;        // [N, channels, coeffs] float32
    Tensor shN;        // [N, channels, coeffs] float32
    ...
    PointCloudNew(Tensor positions, Tensor point_colors)
        : means(std::move(positions)), colors(std::move(point_colors)) {
    ...
    PointCloudNew to_device(Device device) const {
        ...
        if (means.is_valid()) pc.means = means.to(device);
        if (colors.is_valid()) pc.colors = colors.to(device);
        
        // Move Gaussian attributes if present
        if (normals.is_valid()) pc.normals = normals.to(device);
        if (sh0.is_valid()) pc.sh0 = sh0.to(device);
        if (shN.is_valid()) pc.shN = shN.to(device);
        if (opacity.is_valid()) pc.opacity = opacity.to(device);
        if (scaling.is_valid()) pc.scaling = scaling.to(device);
        if (rotation.is_valid()) pc.rotation = rotation.to(device);
    ...
    bool is_valid() const {
        ...
        if (is_gaussian()) {
            ...
            if (sh0.is_valid() && sh0.ndim() != 3) {
                LOG_ERROR("PointCloudNew: sh0 must be 3D");
                return false;
            }
            
            if (shN.is_valid() && shN.ndim() != 3) {
                LOG_ERROR("PointCloudNew: shN must be 3D");
                return false;
            }
    ...
    PointCloudNew clone() const {
        ...
        if (means.is_valid()) pc.means = means.clone();
        if (colors.is_valid()) pc.colors = colors.clone();
        if (normals.is_valid()) pc.normals = normals.clone();
        if (sh0.is_valid()) pc.sh0 = sh0.clone();
        if (shN.is_valid()) pc.shN = shN.clone();
        if (opacity.is_valid()) pc.opacity = opacity.clone();
        if (scaling.is_valid()) pc.scaling = scaling.clone();
        if (rotation.is_valid()) pc.rotation = rotation.clone();
