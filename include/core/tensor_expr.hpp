...
namespace ops {
    template<typename F, typename G> struct composed_unary_op;
}
...
template<typename InputExpr, typename UnaryOp> class UnaryExpr;
...
public:
    UnaryExpr(InnerExpr inner, OuterOp outer, TensorShape shape, Device device, DataType dtype)
        : inner_expr_(std::move(inner)), outer_op_(outer),
          shape_(std::move(shape)), device_(device), dtype_(dtype) {}
...
public:
    PermutationExpr(InputExpr input, IndexExpr indices,
                    TensorShape shape, Device device, DataType dtype)
        : input_(std::move(input)), indices_(std::move(indices)),
          shape_(std::move(shape)), device_(device), dtype_(dtype) {}
...
public:
    UnaryExpr(PermExpr perm, UnaryOp op, TensorShape shape, Device device, DataType dtype)
        : perm_expr_(std::move(perm)), op_(op),
          shape_(std::move(shape)), device_(device), dtype_(dtype) {}
