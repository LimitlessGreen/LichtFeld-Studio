...
namespace gs {
    ...
    class SplatDataRef {
    public:
        SplatDataRef() = default;
        ~SplatDataRef();

        // Delete copy operations
        SplatDataRef(const SplatDataRef&) = delete;
        SplatDataRef& operator=(const SplatDataRef&) = delete;

        // Custom move operations (needed because of mutex)
        SplatDataRef(SplatDataRef&& other) noexcept;
        SplatDataRef& operator=(SplatDataRef&& other) noexcept;
        ...
        static std::expected<SplatDataRef, std::string> init_model_from_pointcloud(
            ...
            const PointCloudRef& point_cloud);
        ...
        SplatDataRef& transform(const glm::mat4& transform_matrix);
        ...
        SplatDataRef crop_by_cropbox(const gs::geometry::BoundingBox& bounding_box) const;
    ...
    private:
        ...
        PointCloudRef to_point_cloud() const;
